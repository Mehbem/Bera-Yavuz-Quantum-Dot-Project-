...
num_steps = 5000; % Number of steps for the random walk
start_position = [0, 0]; % Start from an arbitrary point away from the center
positions = start_position; % Initialize positions array


% Function for stochastic movement
function [x_pos_new, y_pos_new] = new_stochastic_movement(x_pos, y_pos, dir_x, dir_y,ANC300,Frequency)
...
for step = 1:num_steps
    ...
    [x_pos_new, y_pos_new] = new_stochastic_movement(x_pos, y_pos, dir_x, dir_y);
    ...
    if photon_counts < 0
        [x_pos_new, y_pos_new] = new_stochastic_movement(x_pos_new, y_pos_new, -dir_x, -dir_x);
    end
