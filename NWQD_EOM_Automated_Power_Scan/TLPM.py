import os
from ctypes import cdll, c_long, c_uint32, byref, create_string_buffer, c_bool, c_char_p, c_int, c_int16, c_double, sizeof, c_void_p

class TLPM:

	def __init__(self):
		if sizeof(c_void_p) == 4:
			#os.getcwd(): get the current working directory(use absolute path for avoiding error)
			#thus requires users to place TLPM_32.dll under the same folder as the project
			self.dll = cdll.LoadLibrary(os.getcwd()+"//TLPM_32.dll")
		else:
			self.dll = cdll.LoadLibrary(os.getcwd()+"//TLPM_64.dll")
